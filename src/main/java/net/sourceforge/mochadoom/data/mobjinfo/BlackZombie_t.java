...
public class BlackZombie_t extends Zombie_t {
  
  public BlackZombie_t() {
    super(3004,StateNum.S_POSS_STND,20,StateNum.S_POSS_RUN1,
        ...
        20,20 * FRACUNIT,56 * FRACUNIT,100,0,sfxenum_t.sfx_posact,
        MF_SOLID | MF_SHOOTABLE | MF_COUNTKILL,StateNum.S_POSS_RAISE1);
  }


  public BlackZombie_t(int doomednum2, StateNum spawnstate2, int spawnhealth2, StateNum seestate2, sfxenum_t seesound2,
  ...
  public <V> V getColorMap(LightsAndColors<V> colormaps){
    if(this.colormap == null){
      ...
      for(int i=0; i < 256; i++){
        colormap[i] = (short) ((BLACKCOLOR*9999 + ((short[]) colormaps.colormaps[colormaps.colormaps.length - 1])[i])/10000);
      }
    }
    return (V) colormap;
  }
