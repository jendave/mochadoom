...
import net.sourceforge.mochadoom.doom.wbplayerstruct_t;
import net.sourceforge.mochadoom.doom.wbstartstruct_t;
...
import static net.sourceforge.mochadoom.video.DoomVideoRenderer.SCREEN_BG;
import static net.sourceforge.mochadoom.video.DoomVideoRenderer.SCREEN_FG;
...
//
    ...
    protected void unloadData() {
        ...
        W.UnlockLumpNum(wiminus);
        wiminus = null;

        for (i = 0; i < 10; i++) {
            W.UnlockLumpNum(num[i]);
            num[i] = null;
        }

        if (DS.isCommercial()) {
            for (i = 0; i < NUMCMAPS; i++) {
                W.UnlockLumpNum(lnames[i]);
                lnames[i] = null;
            }
        } else {
            W.UnlockLumpNum(yah[0]);
            yah[0] = null;
            W.UnlockLumpNum(yah[1]);
            yah[1] = null;

            W.UnlockLumpNum(splat);
            splat = null;
        ...
        W.UnlockLumpNum(percent);
        percent = null;
        W.UnlockLumpNum(colon);
        colon = null;
        W.UnlockLumpNum(finished);
        finished = null;
        W.UnlockLumpNum(entering);
        entering = null;
        W.UnlockLumpNum(kills);
        kills = null;
        W.UnlockLumpNum(secret);
        secret = null;
        W.UnlockLumpNum(sp_secret);
        sp_secret = null;
        W.UnlockLumpNum(items);
        items = null;
        W.UnlockLumpNum(frags);
        frags = null;
        W.UnlockLumpNum(time);
        time = null;
        W.UnlockLumpNum(sucks);
        sucks = null;
        W.UnlockLumpNum(par);
        par = null;
        W.UnlockLumpNum(victims);
        victims = null;
        W.UnlockLumpNum(killers);
        killers = null;
        W.UnlockLumpNum(total);
        total = null;
