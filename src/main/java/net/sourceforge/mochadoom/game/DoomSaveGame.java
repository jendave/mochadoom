package net.sourceforge.mochadoom.game;
...
import static net.sourceforge.mochadoom.data.Limits.SAVESTRINGSIZE;
import static net.sourceforge.mochadoom.data.Limits.VERSIONSIZE;
...
public class DoomSaveGame
    ...
    public void unpack(ByteBuffer buf)
        ...
        if (buf.get() != 0x1d) properend = false;
        else
            properend = true;
        buf.reset();
    ...
    public void read(DataInputStream f)
        ...
        f.skip(available - 1);
        if (f.readByte() != 0x1d) properend = false;
        else
            properend = true;
